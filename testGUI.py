...
def mainloop(surface, N, dcities, scities, start_timer):
    ...
    while True:
        ...
        if VERBOSE and iters % 10000 == 0:
            print
            "N/iters/beta/s_energy_min =", N, iters, beta, round(s_energy_min, 3)
        ...
        if d_energy < d_energy_min:
            ...
            if VERBOSE:
                print
                "Tour length direct sampling:", d_energy_min, "at iteration", iters
            dcities = new_dcities[:]
            diters = iters
            change = True
        ...
        if new_route:
            if VERBOSE:
                print
                "Tour length simulated annealing:", s_energy_min, "at iteration", iters
            scities = new_scities[:]
            siters = iters
            change = True
        ...
        if change:
            ...
            draw_text(surface, helv24, "https://github.com/RandyWaterhouse/TSP_simulated_annealing",
                      (SIZE // 2, SIZE + STATUS_HEIGHT + STATUS_HEIGHT2 + DELIM_WIDTH + 5), COLORS["DARKGREY"])
